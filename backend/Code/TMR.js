...
async function fetchTMR() {
    const url = `https://api.qldtraffic.qld.gov.au/v2/events?apikey=TXJtVfQrhl5FY9LRwadD4aDB5ghSK75U9qZfLtTY`;

    try {
        const response = await fetch(url);
        if (!response.ok) {
            throw new Error(`Error fetching from TMR: ${response.status}`);
        }
        const jsonData = await response.json();
        return jsonData;
    } catch (error) {
        console.error("Error fetching data from TMR: ", error);
        throw error;
    }
}
    ...
    try {
        if (eventsResponse && Array.isArray(eventsResponse.features)) {
            ...
            for (const feature of eventsResponse.features) {
                if (
                    (dataPrefix === 'QFES' && 
                        (feature.properties.GroupedType === 'RESCUE ROAD CRASH' || feature.properties.GroupedType === 'FIRE VEHICLE')) ||
                    (dataPrefix === 'AusRoads' && feature.properties.event_type === 'Crash' && feature.properties.source.provided_by !== "Transport for NSW" && feature.geometry.type !== 'GeometryCollection' && feature.geometry.type !== 'MultiLineString')
                ) {
                    ...
                    const locality = feature.properties.Locality || feature.properties.road_summary.locality.toUpperCase();
                    feature.locality = locality;

                    const location = feature.properties.Location || feature.properties.road_summary.road_name;
...
(async () => {
    try {
        ...
        setInterval(async () => {
            try {
                const data = await fetchTMR();
                await checkAndUpdateEvents(data, folderPath, 'AusRoads');
            } catch (error) {
                console.error('Error in TMR loop:', error);
            }
